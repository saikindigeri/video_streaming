...
const Saved: React.FC = () => {
  ...
  const { savedVideos }: { savedVideos: Video[] } = useContext(ThemeAndVideoContext);
  const { isDarkTheme }: { isDarkTheme: boolean } = useContext(ThemeAndVideoContext);
          ...
          {savedVideos.length > 0 ? (
            savedVideos.map((eachVideo: Video) => (
              <Link href={`videos/${eachVideo.id}`} key={eachVideo.id} className="p-1 w-70 m-2 list-none flex flex-col">
                <div>
                  <Image height={500} width={500} alt="thumbnail" src={eachVideo.thumbnailUrl} />
                  <div className="flex">
                    <Image height={30} width={30} alt="profile" className="h-[30px] m-5" src={eachVideo.profileImage} />
